      ...
      <section className="relative mx-auto flex min-h-screen w-full max-w-5xl flex-col justify-center px-6 py-20 sm:px-10">
        <div className="w-full rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl shadow-black/50 backdrop-blur-xl sm:p-10">
          ...
          <p className="mt-4 max-w-2xl text-pretty text-base leading-7 text-zinc-300 sm:text-lg">
            YouTube ders videolarindaki kritik dakikalari saniyeler icinde bul, o ana atla ve sinava
            odaklan.
          </p>

          <div className="mt-10 flex w-full flex-col gap-3">
            <label htmlFor="youtube-link" className="text-sm text-zinc-400">
              YouTube linki
            </label>
          ...
          {result ? (
            <div className="mt-10 space-y-8 border-t border-white/10 pt-10">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {result.meta.transcriptTruncatedForModel ? (
                  <span className="text-xs text-amber-200/90">
                    Uzun video: modele giden transkript kısaltıldı; özet yine üretildi.
                  </span>
                ) : null}
              </div>
              ...
              <div>
                ...
                <ul className="mt-3 space-y-3">
                  {result.analysis.criticalMoments.map((m) => (
                    <li
                      key={`${m.startSeconds}-${m.title}`}
                      className="rounded-2xl border border-rose-400/25 bg-rose-500/5 p-4"
                    >
                      <a
                        href={youtubeDeepLink(result.videoId, m.startSeconds)}
                        target="_blank"
                        rel="noreferrer"
                      ...
                      >
                        ...
                        <span className="font-normal text-zinc-400">
                          ({Math.floor(m.startSeconds / 60)}:
                          {(Math.floor(m.startSeconds) % 60).toString().padStart(2, "0")})
                        </span>
                      </a>
              ...
              <div>
                <h2 className="text-lg font-semibold text-white">Transkript (önizleme)</h2>
                <p className="mt-1 text-xs text-zinc-500">
                  {result.transcript.totalCues} satır
                  {result.transcript.truncated ? " — ekranda ilk 200 satır" : ""}
                </p>
